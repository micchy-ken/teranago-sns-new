...
async function startServer() {
  ...
  // ==========================================
  // iCal (ICS) カレンダー連携・外部エクスポート API
  // ==========================================
  const handleIcalExport = (req: express.Request, res: express.Response) => {
    try {
      const filename = req.params.filename || '';
      let userId = '';
      const match = filename.match(/user_([^_]+)_calendar\.ics/i) || filename.match(/user_([^_]+)\.ics/i);
      if (match) {
        userId = match[1];
      ...
      // カレンダー名（URLクエリ ?name= または ?calname=、なければデフォルト）
      const queryName = (req.query.name || req.query.calname || '') as string;
      const calendarName = (queryName.trim() || `社内予定 (ID:${userId || '全体'})`).replace(/[\r\n]/g, '');

      // RFC 5545 iCalendar content
      const now = new Date();
      ...
      const icsLines = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        'PRODID:-//TERANAGO SNS//Calendar Feed//JA',
        'CALSCALE:GREGORIAN',
        'METHOD:PUBLISH',
        `X-WR-CALNAME:${calendarName}`,
        `NAME:${calendarName}`,
        'X-WR-TIMEZONE:Asia/Tokyo',
        `X-WR-CALDESC:TERANAGO SNS 社内カレンダー同期 (${calendarName})`,
      ...
      res.setHeader('Content-Disposition', `inline; filename="calendar_${userId || 'events'}.ics"`);
      ...
      res.status(500).send('iCal generation error');
    }
  };

  app.get('/ical/:filename', handleIcalExport);
  app.get('/api/ical/:filename', handleIcalExport);
  app.get('/api/ical/export.ics', handleIcalExport);
  app.get('/ical-export', handleIcalExport);

  // iCal Proxy API (for fetching external ICS with CORS support)
  app.get('/api/ical-proxy', async (req, res) => {
    ...
    try {
      ...
      const response = await fetch(normalized, {
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; TERANAGO-SNS/1.0; iCalFetcher)'
        }
      });
      if (!response.ok) {
        return res.status(response.status).send(`Failed to fetch remote iCal: ${response.statusText}`);
      }
      ...
      res.status(500).send(e.message || 'iCal Proxy failed');
    }
  });
