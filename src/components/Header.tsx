        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative" ref={themePickerRef}>
            <button
              ...
              className={`p-2.5 rounded-full transition-colors flex items-center justify-center gap-1 cursor-pointer ${
                isThemePickerOpen
                  ? 'bg-indigo-50 ring-2 ring-indigo-500/20'
                  : 'hover:bg-slate-100'
              }`}
              title="画面の配色・テーマ切り替え（くすみカラー/彩度調整）"
            >
              <div className="flex items-center gap-0.5" title="配色トーン切替">
                <span className="w-3.5 h-3.5 rounded-full bg-indigo-600 inline-block shadow-2xs ring-1 ring-white" />
                <span className="w-2.5 h-2.5 rounded-full bg-amber-500 inline-block shadow-2xs ring-1 ring-white -ml-1.5" />
                <span className="w-2.5 h-2.5 rounded-full bg-rose-500 inline-block shadow-2xs ring-1 ring-white -ml-1.5" />
              </div>
            </button>

            {isThemePickerOpen && (
              <div className="absolute right-0 top-full mt-2 w-72 sm:w-80 bg-white rounded-2xl shadow-2xl border border-slate-200/90 z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-150 p-3 ring-1 ring-slate-900/5">
                <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-100">
                  <div className="flex items-center gap-1.5 font-bold text-xs text-slate-800">
                    <Palette className="w-4 h-4 text-indigo-600" />
                    <span>画面の配色・トーン設定</span>
                  </div>
                  <span className="text-[10px] text-slate-400 font-medium">彩度・色味の調整</span>
                </div>
                      ...
                      >
                        <div className="flex items-center gap-1 shrink-0 mt-0.5">
                          {opt.previewColors.map((color, idx) => (
                            <span
                              key={idx}
                              className="w-3 h-3 rounded-full shadow-2xs border border-white/80 inline-block"
